...
Token-leak protocol — running the `.gitignore` check + working-tree scan + staged-diff scan, and the abort-on-match behaviour — is defined in `CLAUDE.md` under "Commit safety — secret scan". Apply that protocol before every push, commit, or GitHub API write in this mission.
...
Use this when the working directory is a clone of the repo with an SSH remote and a working SSH key. Run the SSH probe per `CLAUDE.md` → "Push protocol — SSH probe" (`git rev-parse --is-inside-work-tree`, `git remote get-url origin`, `git ls-remote origin HEAD`).
